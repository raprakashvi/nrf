...
int32_t data_store[8000] = {0};
int32_t data_read[8000] = {1};
 //( 9*2043 -3)
int32_t MAX_SEND_LENGTH = 2043 ;// per page limit
int32_t DATA_STORE_LEN = 8000;
int j = 0;
int32_t intValueMgZ;
...
static ret_code_t fds_write(void)
{
		...
		static int32_t m_deadbeef[8000] = {0} ; // changing size of deadbeef to match the size of total length of the array
		...
		fds_record_desc_t   record_desc;
                ...
                // Set up record.
                  ...
                  //else {
                  //  record.data.length_words = ((  + 3) / sizeof(uint32_t));
                  //      }

                  record.data.length_words = MAX_SEND_LENGTH;
                 // record.data.p_data          = &m_deadbeef[DATA_STORE_LEN - write_counter]; //DATA_STORE_LEN = 8000
...
{ 
 ...
 static void acc_data(void)   //static void acc_data(int intValueMgZ , int i)
 {
   ...
   for(int loop = 0; loop < 8000 ; loop++)
    ...
    // printf("%d\t%d ", loop,data_store[loop]);
    // printf("\r\n");
    // }
 ...
 static void data_grab(){
         ...
         {     
                ...
                //acc_data();
               ready_to_write = true;
                //err_code = app_timer_start(m_single_shot_timer_id, APP_TIMER_TICKS(10000), NULL);
                //APP_ERROR_CHECK(err_code);
...
static void single_shot_timer_handler(void * p_context)
{
    ...
    printf("Inside single shot timer");
